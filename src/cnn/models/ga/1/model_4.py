...
def create_model(input_size):
    ...
    x.add(layers.BatchNormalization())
    x.add(layers.Conv2D(17, (1, 1), activation='sigmoid', padding='same'))
    x.add(layers.BatchNormalization())
    x.add(layers.BatchNormalization())
    ...
    x.add(layers.Dense(1882, activation='sigmoid'))
    x.add(layers.Dense(1387, activation='relu'))
    x.add(layers.Dense(1173, activation='leaky_relu'))
