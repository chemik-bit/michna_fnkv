...
def create_model(input_size):
    ...
    x.add(layers.Conv2D(55, (2, 2), activation='tanh', padding='same', input_shape=(input_size[0], input_size[1], 1)))
    ...
    x.add(layers.BatchNormalization())
    x.add(layers.Conv2D(15, (7, 7), activation='sigmoid', padding='same'))
    x.add(layers.BatchNormalization())
    x.add(layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
    x.add(layers.BatchNormalization())
    ...
    x.add(layers.Dense(1778, activation='sigmoid'))
