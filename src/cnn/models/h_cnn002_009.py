...
def create_model(input_size):
    ...
    x.add(layers.Conv2D(120, (3, 3), activation="relu", padding="same", input_shape = (input_size[0], input_size[1], 1)))
    x.add(layers.BatchNormalization())
    x.add(layers.MaxPooling2D(pool_size=(2, 2)))
    x.add(layers.Conv2D(120, (3, 3), activation="relu", strides=(1, 1)))
    x.add(layers.BatchNormalization())
    x.add(layers.MaxPooling2D(pool_size=(2, 2)))
    x.add(layers.Conv2D(120, (3, 3), activation="relu", strides=(1, 1)))
    x.add(layers.BatchNormalization())
    ...
    x.add(layers.Dense(1024, activation="relu"))
