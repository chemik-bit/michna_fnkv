...
def create_model(input_size):
    ...
    x.add(layers.Conv2D(25, (7, 7), activation='relu', padding='same', input_shape=(input_size[0], input_size[1], 1)))
    ...
    x.add(layers.BatchNormalization())
    x.add(layers.BatchNormalization())
    x.add(layers.Conv2D(60, (3, 3), activation='sigmoid', padding='same'))
    x.add(layers.MaxPooling2D(pool_size=(1, 1), strides=(2, 2)))
    x.add(layers.BatchNormalization())
    ...
    x.add(layers.Dense(1049, activation='leaky_relu'))
