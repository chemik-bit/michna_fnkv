...
def unique_pairs(pairs_in_file: int = 4000):
    """
    Wrapping function.
    :param pairs_in_file:
    :type pairs_in_file:
    ...
    :rtype:
    """
    def make_pairs(images_paths: Path, path_to_save: Path, pairs_in_file: int = 4000):
        """
        Load images and make pairs for siamese network and save pickle it.
        :param pairs_in_file: number of pairs in file. Corresponds to subset size!
    ...
    for item in pickled_sets.values():
        ...
        if len(image_paths) > 0:
            make_pairs(image_paths, item[0], pairs_in_file)
        else:
            print(f"{Fore.RED}Missing paths in {item[1]}{Style.RESET_ALL}")