...
def create_model(input_size):
    ...
    x.add(layers.Conv2D(38, (7, 7), activation='tanh', padding='same', input_shape=(input_size[0], input_size[1], 1)))
    x.add(layers.MaxPooling2D(pool_size=(1, 1), strides=(2, 2)))
    x.add(layers.BatchNormalization())
    x.add(layers.Conv2D(32, (4, 4), activation='relu', padding='same'))
    x.add(layers.Conv2D(2, (4, 4), activation='relu', padding='same'))
    ...
    x.add(layers.Dense(1912, activation='tanh'))
    x.add(layers.Dropout(0.7))
    x.add(layers.Dense(74, activation='relu'))
