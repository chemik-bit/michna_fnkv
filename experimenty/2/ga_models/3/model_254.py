...
def create_model(input_size):
    ...
    x.add(layers.BatchNormalization())
    x.add(layers.Conv2D(8, (2, 2), activation='leaky_relu', padding='same'))
    x.add(layers.BatchNormalization())
    x.add(layers.MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
    x.add(layers.GlobalAveragePooling2D())
    x.add(layers.Dense(1096, activation='relu'))
    x.add(layers.Dense(1840, activation='leaky_relu'))
    x.add(layers.Dropout(0.7))
    x.add(layers.Dense(713, activation='relu'))
